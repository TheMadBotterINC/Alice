...
class ConnectorTest < ActiveSupport::TestCase
  ...
  test "should accept looking_glass as valid connector type" do
    looking_glass = Connector.new(
      name: "Unique Test Looking Glass",
      connector_type: "looking_glass",
      config: {
        "api_url" => "https://looking-glass.example.com",
        "api_key" => "lg_test_key",
        "connection_id" => "123"
      }
    )

    assert looking_glass.valid?, "Looking Glass connector should be valid, but had errors: #{looking_glass.errors.full_messages}"
  end

  test "should validate looking_glass config has required fields" do
    looking_glass = Connector.new(
    ...
    assert_not looking_glass.valid?
    assert_includes looking_glass.errors[:config], "missing required fields for looking_glass: api_key, connection_id"
  end

  test "looking_glass should be destination only" do
    looking_glass = Connector.new(
      name: "Test Looking Glass",
      connector_type: "looking_glass",
      config: {
    ...
    assert looking_glass.destination_only?
  end

  test "should create LookingGlassAdapter for looking_glass connector" do
    looking_glass = Connector.new(
      name: "Test Looking Glass",
      connector_type: "looking_glass",
      config: {
    ...
    adapter = looking_glass.adapter
    assert_instance_of ConnectorAdapters::LookingGlassAdapter, adapter
  end
end
